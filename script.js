...
async function loadSessions() {
    console.log('🔄 Ładowanie sesji...');
    
    // Najpierw załaduj z localStorage
    sessions = JSON.parse(localStorage.getItem('sessions')) || [];
    console.log('📦 Załadowano z localStorage:', sessions.length);
    renderHistory();
    updateTodayStats();
    
    // Potem spróbuj z Firebase
    try {
        ...
        if (firebaseSessions.length > 0) {
            ...
            console.log('☁️ Załadowano z Firebase:', sessions.length);
            renderHistory();
            updateTodayStats();
        }
    } catch (error) {
        console.error('❌ Błąd Firebase:', error.message);
        console.log('✅ Używam danych lokalnych');
    }
}

// Event Listeners
startBtn.addEventListener('click', () => {
    console.log('START kliknięty');
    startTimer();
});
...
async function stopTimer() {
    ...
    console.log('Timer zatrzymany, czas:', elapsedTime);
...
async function saveSession() {
    console.log('=== ROZPOCZĘCIE ZAPISYWANIA SESJI ===');
    console.log('Czas:', elapsedTime, 'ms');
    
    // Sprawdź czy jest czas do zapisania
    if (elapsedTime === 0 || !elapsedTime) {
        console.warn('Brak czasu do zapisania!');
        showNotification('⚠️ Brak czasu do zapisania');
        resetTimer();
        return;
    }
    ...
    console.log('Dane sesji do zapisania:', sessionData);
    
    // ZAWSZE zapisz lokalnie najpierw
    const localSession = {
    ...
    localStorage.setItem('sessions', JSON.stringify(sessions));
    console.log('✓ Zapisano lokalnie');
    
    // Spróbuj zapisać do Firebase
    try {
        console.log('Próba zapisu do Firebase...');
        const docRef = await addDoc(collection(db, 'sessions'), sessionData);
        console.log('✓ Zapisano do Firebase:', docRef.id);
        
        // Zaktualizuj ID na Firebase ID
        const sessionIndex = sessions.findIndex(s => s.id === localSession.id);
        if (sessionIndex !== -1) {
            sessions[sessionIndex].id = docRef.id;
            localStorage.setItem('sessions', JSON.stringify(sessions));
        }
        
        showNotification('✅ Sesja zapisana w chmurze!');
    } catch (error) {
        console.error('✗ Błąd Firebase:', error);
        console.error('Szczegóły:', error.message);
        showNotification('✅ Sesja zapisana lokalnie');
    }
    
    console.log('Aktualna liczba sesji:', sessions.length);
    renderHistory();
    updateTodayStats();
    resetTimer();
    console.log('=== ZAKOŃCZENIE ZAPISYWANIA ===');
}
